...
const getSecondaryTrainingFocusAttributes = (focus: SecondaryTrainingFocus): (keyof PlayerAttributes)[] => {
    switch (focus) {
        ...
        case 'Bolas Paradas de Defesa': return ['heading', 'positioning'];
        case 'Contra-Ataque': return ['pace', 'workRate', 'positioning'];
        case 'Pressão Alta': return ['stamina', 'workRate', 'aggression'];
        default: return [];
    }
};
    ...
    for (const pId in players) {
        ...
        if (Math.random() < developmentChance) {
            ...
            for (let i = 0; i < numImprovements; i++) {
                ...
                weightedAttributes.push(...getSecondaryTrainingFocusAttributes(club.weeklyTrainingFocus.secondary));
        ...
        // --- 5. Mentoring Development ---
        if (player.mentorId && player.age <= 21) {
            const mentor = players[player.mentorId];
            if (mentor) {
                // Base chance for any mentoring effect
                const mentoringChance = ((mentor.attributes.teamwork + mentor.attributes.workRate) / 200) * 0.25; // Max 25% chance per month

                if (Math.random() < mentoringChance) {
                    let improved = false;
                    // Guaranteed boost to Teamwork & WorkRate
                    if (player.attributes.teamwork < 99 && Math.random() < 0.6) {
                        player.attributes.teamwork += 1;
                        player.attributeChanges.push({ date: new Date(currentDate), attr: 'teamwork', change: 1 });
                        improved = true;
                    }
                    if (player.attributes.workRate < 99 && Math.random() < 0.6) {
                        player.attributes.workRate += 1;
                        player.attributeChanges.push({ date: new Date(currentDate), attr: 'workRate', change: 1 });
                        improved = true;
                    }

                    // Positional boost (50% chance if a mental attribute was improved)
                    if (improved && Math.random() < 0.5) {
                        const mentorCategory = getRoleCategory(mentor.naturalPosition);
                        let attributeToBoost: keyof PlayerAttributes | null = null;
                        
                        // Select an attribute based on mentor's specialty
                        if (mentorCategory === 'DEF') {
                            attributeToBoost = Math.random() < 0.5 ? 'tackling' : 'positioning';
                        } else if (mentorCategory === 'MID') {
                            attributeToBoost = Math.random() < 0.5 ? 'passing' : 'creativity';
                        } else if (mentorCategory === 'FWD') {
                            attributeToBoost = Math.random() < 0.5 ? 'shooting' : 'dribbling';
                        }

                        if (attributeToBoost && player.attributes[attributeToBoost] < 99) {
                            player.attributes[attributeToBoost] += 1;
                            player.attributeChanges.push({ date: new Date(currentDate), attr: attributeToBoost, change: 1 });
                        }
                    }
                }
            }
...
export const processPlayerConcerns = (state: GameState): { players: Record<number, Player>; newNewsItems: { headline: string, content: string, type: NewsItem['type'], relatedEntityId?: number }[] } => {
    const newPlayers: Record<number, Player> = JSON.parse(JSON.stringify(state.players));
    const newNewsItems: { headline: string, content: string, type: NewsItem['type'], relatedEntityId?: number }[] = [];
    const playerClubId = state.playerClubId;
    if (!playerClubId) {
        return { players: newPlayers, newNewsItems };
    }

    Object.values(newPlayers).forEach((player: Player) => {
        if (player.clubId !== playerClubId || player.concern || Math.random() > 0.1) { // 10% chance per month to develop a concern
            return;
        }

        let concernType: PlayerConcernType | null = null;
        const potentialConcerns: PlayerConcernType[] = [];

        // Playing time concern
        if (player.satisfaction < 30 && ['Titular', 'Rodízio'].includes(player.squadStatus)) {
            potentialConcerns.push('playing_time');
        } 
        // New contract concern
        if (player.morale > 80 && player.contractExpires < new Date(state.currentDate.getFullYear() + 1, state.currentDate.getMonth(), state.currentDate.getDate())) {
            potentialConcerns.push('new_contract');
        }
        // Squad status concern
        if (player.satisfaction > 80 && player.morale > 75 && player.squadStatus === 'Rotação') {
             potentialConcerns.push('squad_status');
        }
        // Wants to be starter
        if (player.morale > 85 && player.squadStatus === 'Titular' && player.satisfaction < 50) {
            potentialConcerns.push('wants_to_be_starter');
        }

        if (potentialConcerns.length > 0) {
            concernType = pickRandom(potentialConcerns);
        }
        ...
        if (concernType) {
            ...
            const concernDef = CONCERN_DEFINITIONS[concernType];
            if (concernDef) {
                newNewsItems.push({
                    headline: `Preocupação do Jogador: ${player.name}`,
                    content: concernDef.getPlayerStatement(player.name),
                    type: 'player_concern',
